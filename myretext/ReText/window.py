...
from ReText.newProject import InputdialogDemo
from ReText.createtable import Table
...
from PyQt5.QtWidgets import QAction, QActionGroup, QApplication, QCheckBox, \
 ...
 QLineEdit, QMainWindow, QMenu, QMessageBox, QTabWidget, QToolBar,QSplitter,QTextEdit,QHBoxLayout,QWidget,QLabel
from PyQt5.QtPrintSupport import QPrintDialog, QPrintPreviewDialog, QPrinter
from dirview import TreeView
import os
from PyQt5.QtCore import pyqtSignal

class ReTextWindow(QMainWindow):

	send_picture_signal = pyqtSignal(str)
	def __init__(self,parent=None):
		...
		self.initTabWidget()
		self.dirTree = TreeView(self)
		self.dirTree.setPath(self.projectPath)
		self.dirTree.setColumnHidden(1,True)
		self.dirTree.setColumnHidden(2,True)
		self.dirTree.setColumnHidden(3,True)
		self.dirTree.resize(100, 480)
		self.tabWidget.resize(540,480)
		self.tabWidget.resize(540,480)
		self.splitter = QSplitter(Qt.Horizontal)
		self.splitter.addWidget(self.dirTree)
		self.splitter.addWidget(self.tabWidget)
		self.dirTree.setVisible(False)
		self.dirTree.isVisible()
		...
		self.actionNewProject = self.act(self.tr("New project"),"document-newpro",self.createNewProject)
		self.actionNewProject.setPriority(QAction.LowPriority)
		self.actionNewProject.setPriority(QAction.LowPriority)
		self.actionOpenProject = self.act(self.tr("Open Project"),"document-open",self.openProject)
		self.actionOpenProject.setPriority(QAction.LowPriority)

		self.actionCreateTable = self.act(self.tr("Create Table"),"",self.createTable)
		self.actionlinedividers = self.act(self.tr("水平分割线"),"",self.Horizontaldividers)
		# self.actionCreateTable.setProperty(QAction.LowPriority)
		...
		self.actionuploadiimg = self.act(self.tr(''),'document-upload',self.send_picture)
		self.actionOpen.setPriority(QAction.LowPriority)
		...
		menuPar = menubar.addMenu(self.tr('段落'))
		menuPar.addAction(self.actionCreateTable)
		menuPar.addAction(self.actionlinedividers)
		...
		toolBar.addAction(self.actionuploadiimg)
	...
	def Horizontaldividers(self):

		cursor = self.currentTab.editBox.textCursor()
		line_string = '\n----'
		cursor.insertText(line_string)
	...
	def openProject(self):
		...
		for projectName in projectNameList:
			if QFile.exists(projectName):
				itemlist.append(projectName)
			else:
				index = projectNameList.index(projectName)
				del gitCloneUrlList[index]
		if not len(itemlist):
			QMessageBox.about(self, "sorry", "sorry,you have none project")
			return
		self.projectPath, ok = QInputDialog.getItem(self, "select project", "project list", itemlist, 0, False)

		if ok and self.projectPath:
			self.dirTree.setPath(self.projectPath)
			self.dirTree.setVisible(True)
			self.dirTree.update()
			self.dirTree.current_select_path = None
			self.update()
			if self.projectPath in itemlist:
				index = itemlist.index(self.projectPath)
				itemlist.remove(self.projectPath)
				gitCloneUrl = gitCloneUrlList[index]
			itemlist.insert(0, self.projectPath)
			gitCloneUrlList.insert(0, gitCloneUrl)
			writeListToSettings("rencentProject", itemlist)
	...
	def insertFormatting(self, formatting):
		...
		toinsert = {
			...
			'image': ('![', text or self.tr(''), c, '](', self.tr(''), ')'),
	...
	def send_picture(self):
		...
		# print('fname:',fname)
		self.send_picture_signal.emit(fname)
